...
def accuracy2(scores, labels):
    r"""
        ...
        labels: torch.LongTensor, shape (B?, N, 1)
            ground truth labels
    ...
    """
    num_classes = scores.size(1)  # Get the number of classes

    predictions = torch.max(scores, dim=1).indices

    accuracies = []

    accuracy_mask = predictions == labels.squeeze()
...
def intersection_over_union2(scores, labels):
    r"""
        Compute the per-class IoU and the mean IoU # TODO: complete doc
    ...
    for label in range(num_classes):
        pred_mask = predictions == label
        labels_mask = labels.squeeze() == label
        intersection = (pred_mask & labels_mask).float().sum()
        union = (pred_mask | labels_mask).float().sum()
